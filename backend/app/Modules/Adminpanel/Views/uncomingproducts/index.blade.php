...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            ПОДАТЬ ЗАЯВКУ НА РАСХОД
        </h1>
        <ol class="breadcrumb">
            <li><a href="/admin"><i class="fa fa-dashboard"></i>Главная</a></li>
            <li class="active">Заявки на расход</li>
        </ol>
    </section>
                ...
                <div id="home" class="tab-pane fade in active">
                    <h2>СПИСОК ЗАЯВОК</h2>
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <td><?php if ($uncomingproduct->type == 0) {echo 'Внешняя';}else{echo 'Внутренняя';} ?></td>
                                <td><?php if ($uncomingproduct->status == 0) {echo 'Неактивная';}else{echo 'Активная';} ?></td>
                                <td>{{$uncomingproduct->name}}</td>
                                <td>{{$uncomingproduct->created_at}}</td>
                                <td>{{$uncomingproduct->department}}</td>
                                <td>{{$uncomingproduct->exdepartment}}</td>
                                <td><a href="{{ route('adminpanel.uncomingproducts.edit', $uncomingproduct->id) }}" class="fa fa-pencil"></a>
                                <form action="{{ route('adminpanel.uncomingproducts.destroy', $uncomingproduct->id) }}" method="POST">
                                    @csrf
                                    @method('delete')
