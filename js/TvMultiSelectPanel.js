...
(function ($) {
    $.fn.panelMultiSelect = function (options) {
        ...
        var defaults = {
            ...
            elementClassName: 'btn btn-success btn-lg',
            elementShowClassName: 'tv-select-element_show',
            selectPanelContentWrapperClassName: 'tv-select-panel-content-wrapper',
            selectedElementCss: {color: 'white', background: '#F13535', border: ''},
            notSelectedElementCss: {color: 'white', background: '#00a65a', border: ''},
            selectedCountColor: '#608001',
            selectedElementStyle: 'color: white; background: #F13535; border: ',
            notSelectedElementStyle: 'color: white; background: #00a65a; border: ',
        ...
        function renderUiFromData() {
            ...
            const halfBeforeStringBtn_1 = '<button style="margin:10px;';
            ...
            $(dataSource).each(function () {
                ...
                buttonElementListString += halfBeforeStringBtn_1 + notSelectedElementStyle + halfBeforeStringBtn_2 + '" id ="' +
                    keyData + '">' + valueData + '</button>';

                selectedButtonElementListString += halfBeforeStringBtn_1 + selectedElementStyle + halfBeforeStringBtn_2 + '" id ="' +
                    keyData + '">' + valueData + '</button>';
            });
            ...
            '<div><span class="btn-lg btn-primary btn-xs-custom" id="' + SELECT_ALL_BTN_ID +
            '">Выбрать все</span><span class="blue-span">&nbsp;/</span> <span class="btn-xs-custom btn-lg btn-primary" id="' +
            UN_SELECT_ALL_BTN_ID + '">Cнять все</span>' +
            '</div></div>' +
            '<div class="align-height-center-right">' +
            '<span class="input-group search-input-group"><input class="input-search" id="' + SEARCH_INPUT_ID +
            '" placeholder="Искать товар..." type="text"/><span id="' + SEARCH_CLEAR_ID +
            '" class="clear-search">x</span><span style="border:none;" id="' + SEARCH_BTN_ID +
            '" class="btn input-group-addon fa fa-search search-icon"></span></span>' +
            '</div><div class="clearfix"></div></div>';

                /* TO DO COUNT PRODUCTS
                <div class="' + settings.tvSelectFooterClassName + '">' +
                '<div style="float: right" class="footer-height-center">' +
                '<span>Выбрано: <span style="color: ' + settings.selectedCountColor +
            ...
            const footerMultiSelectString = '';

            const contentMultiSelectString = '<div style="width: ' +
                ($(elementReference).width() - 5) + 'px" class="' + settings.selectPanelContentWrapperClassName +
                '" id="' + settings.selectPanelContentWrapperClassName + '"><div id="' + BUTTON_WRAPPER_ID +
                '" style="float:left;">' + buttonElementListString + '</div><div class="clearfix"></div></div>';
        ...
        function setupNoSelectButton(buttonElement) {
            ...
            $(buttonElement).each(function (index) {
                ...
                jQuery.each(prod_array, function(index) {
                    if(this.id == key) {
                        delete_prod_on_modal(index);
                    }
        ...
        function setupEventSearching() {
            ...
            let searchContent = '<div class="search-header"><div class="search-header-right"><button id="' +
                EXIST_SEARCH_BTN_ID + '" class="btn-lg btn-primary btn-xs-custom">Выйти из режима поиска</button></div><div class="search-header-left">' +
                (searchTxt.length !== 0 ? '- ' + '<span style="font-weight: bold">' + countResult + '</span>&nbsp;товаров найдено в запросе "' +
